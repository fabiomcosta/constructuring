...
var DeclarationWrapper = function(path) {
  ...
  if (!this._isVarDeclarator && !n.SequenceExpression.check(parent)) {
    parent = this._sequenceExpression = b.sequenceExpression([]);
    this._replacements = parent.expressions;
  } else {
    this._replacements = [];
  }
...
DeclarationWrapper.prototype.isAlreadyDeclared = function(identifier) {
  for (var i = 0; i < this._replacements.length; i++) {
    var declaration = this._replacements[i];
